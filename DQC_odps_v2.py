...
def read_data_dict():
    ...
    data_dict = DataFrame(data_dict_table)
    return data_dict
...
def check_data_quality(df, table_name, fields, pk_fields, field_types):
    ...
    if pk_fields:
        ...
        duplicate_count = df.groupby(pk_columns).count().filter(lambda x: x['count'] > 1).count()
    ...
    # 3. 检查空值占比
    for field in fields:
        null_count = df[df[field].isnull()].count()
        empty_count = df[df[field] == ''].count()
        null_str_count = df[df[field].astype(str).upper() == 'NULL'].count()
    ...
    # 4. 检查枚举值
    for field in fields:
        unique_values = df[field].distinct().count()
        if unique_values < 20:
            issues["enum_values"].add(field)  # 记录有枚举值的字段
            print(f"表 {table_name} 的字段 {field} 的枚举值数量: {unique_values}")
    
    # 5. 类型检查
    for field, field_type in zip(fields, field_types):
        if field_type == 'datetime':
            try:
                df[field].astype('datetime')
            ...
            try:
                amount_values = df[field].astype('int64')
                if amount_values.filter(amount_values <= 0).count() > 0:
